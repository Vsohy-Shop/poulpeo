...
public class Placeholder extends View {
    ...
    private void init(AttributeSet attributeSet) {
        ...
        if (attributeSet != null) {
            TypedArray obtainStyledAttributes = getContext().obtainStyledAttributes(attributeSet, C1303R.styleable.ConstraintLayout_placeholder);
            int indexCount = obtainStyledAttributes.getIndexCount();
            for (int i = 0; i < indexCount; i++) {
                int index = obtainStyledAttributes.getIndex(i);
                if (index == C1303R.styleable.ConstraintLayout_placeholder_content) {
                    this.mContentId = obtainStyledAttributes.getResourceId(index, this.mContentId);
                } else if (index == C1303R.styleable.ConstraintLayout_placeholder_placeholder_emptyVisibility) {
    ...
    public void onDraw(Canvas canvas) {
        if (isInEditMode()) {
            canvas.drawRGB(OAuthErrorCodes.ERR_OAUTH_VERSION, OAuthErrorCodes.ERR_OAUTH_VERSION, OAuthErrorCodes.ERR_OAUTH_VERSION);
