...
public class PathInterpolatorCompat implements Interpolator {
    ...
    /* renamed from: mX */
    private float[] f1096mX;

    /* renamed from: mY */
    private float[] f1097mY;
    ...
    private void initPath(Path path) {
        ...
        if (min > 0) {
            this.f1096mX = new float[min];
            this.f1097mY = new float[min];
            float[] fArr = new float[2];
            for (int i2 = 0; i2 < min; i2++) {
                pathMeasure.getPosTan((((float) i2) * length) / ((float) (min - 1)), fArr, (float[]) null);
                this.f1096mX[i2] = fArr[0];
                this.f1097mY[i2] = fArr[1];
            }
            if (((double) Math.abs(this.f1096mX[0])) <= 1.0E-5d && ((double) Math.abs(this.f1097mY[0])) <= 1.0E-5d) {
                int i3 = min - 1;
                if (((double) Math.abs(this.f1096mX[i3] - 1.0f)) <= 1.0E-5d && ((double) Math.abs(this.f1097mY[i3] - 1.0f)) <= 1.0E-5d) {
                    float f = 0.0f;
                    int i4 = 0;
                    while (i < min) {
                        float[] fArr2 = this.f1096mX;
                        int i5 = i4 + 1;
                        float f2 = fArr2[i4];
                        if (f2 >= f) {
                            fArr2[i] = f2;
            ...
            sb.append(this.f1096mX[0]);
            sb.append(",");
            sb.append(this.f1097mY[0]);
            sb.append(" end:");
            int i6 = min - 1;
            sb.append(this.f1096mX[i6]);
            sb.append(",");
            sb.append(this.f1097mY[i6]);
    ...
    public float getInterpolation(float f) {
        ...
        int length = this.f1096mX.length - 1;
        int i = 0;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f1096mX[i2]) {
                length = i2;
            } else {
                i = i2;
            }
        }
        float[] fArr = this.f1096mX;
        float f2 = fArr[length];
        float f3 = fArr[i];
        float f4 = f2 - f3;
        if (f4 == 0.0f) {
            return this.f1097mY[i];
        }
        float[] fArr2 = this.f1097mY;
        float f5 = fArr2[i];
