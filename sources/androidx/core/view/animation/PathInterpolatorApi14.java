...
class PathInterpolatorApi14 implements Interpolator {
    ...
    /* renamed from: mX */
    private final float[] f523mX;

    /* renamed from: mY */
    private final float[] f524mY;

    PathInterpolatorApi14(Path path) {
        PathMeasure pathMeasure = new PathMeasure(path, false);
        float length = pathMeasure.getLength();
        int i = ((int) (length / 0.002f)) + 1;
        this.f523mX = new float[i];
        this.f524mY = new float[i];
        float[] fArr = new float[2];
        for (int i2 = 0; i2 < i; i2++) {
            pathMeasure.getPosTan((((float) i2) * length) / ((float) (i - 1)), fArr, (float[]) null);
            this.f523mX[i2] = fArr[0];
            this.f524mY[i2] = fArr[1];
        }
    }
    ...
    public float getInterpolation(float f) {
        ...
        int length = this.f523mX.length - 1;
        int i = 0;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f523mX[i2]) {
        ...
        float[] fArr = this.f523mX;
        float f2 = fArr[length];
        float f3 = fArr[i];
        float f4 = f2 - f3;
        if (f4 == 0.0f) {
            return this.f524mY[i];
        }
        float[] fArr2 = this.f524mY;
        float f5 = fArr2[i];
